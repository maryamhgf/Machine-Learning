...
dataSetPositive["year"] = dataSetPositive["Date"].dt.year
dataSetPositive["is_weakend"] = np.where(dataSetPositive["Date"].dt.dayofweek.isin([5, 6]), 1, 0)
dataSetPositive["Quarter"] = dataSetPositive["Date"].dt.quarter
...
desisionTreeClassifier = DecisionTreeClassifier(max_depth = 17)

baggibgClassifier = BaggingClassifier(base_estimator=desisionTreeClassifier, n_estimators=200, max_samples=0.5, 
        max_features=0.5, bootstrap=True, bootstrap_features=False, oob_score=False, warm_start=False, n_jobs=None, random_state=10, verbose=0)
